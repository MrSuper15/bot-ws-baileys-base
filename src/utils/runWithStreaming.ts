...
export const runWithStreaming = async (threadId: string, assistant: any, ctx: any, provider?: any): Promise<string> => {
    let response = '';
        ...
        return new Promise<void>((resolve, reject) => {
            (async () => {
                try {
                    ...
                    const toolOutputs = await Promise.all(toolCalls.map(tc => getToolOutput(tc, undefined, ctx, provider)));
